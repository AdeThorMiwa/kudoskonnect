...
const setCablePlans = ({ primary_product_id, primary_product_price,  plans }) => {
    planInput.innerHTML = `${plans.map(
        ({ plan_id, name, price }) => `<option selected="${ primary_product_id === plan_id }" value="${plan_id}" price="${price}">${name}</option>`
    ).join("")}`
...
const initCablePlans = async (cable, number) => {
    ...
    try {
        const res = await fetcher.get(`/api/transaction/get-cable-plans?cable=${cable}&number=${number}`);
